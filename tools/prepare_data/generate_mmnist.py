...
def generate_mmnist_testset(root, data_name='mnist', num_objects=[2], save_path=None, length=10000):
    ...
    for i in range(length):
        ...
        images, labels = dataset.generate_moving_mnist(num_digits, dataset.background, return_labels=True)
    ...
    label_save_path = save_path.replace('.npy', '_labels.npy')
...
def main():
    ...
    test_data, label_data = generate_mmnist_testset(root, data_name, num_objects, save_path, length)
    print('generate test set:', test_data.shape)
    print('generate test labels:', label_data.shape)
